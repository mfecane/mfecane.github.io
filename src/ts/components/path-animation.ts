...
const init = (): void => {
  const els = document.querySelectorAll('.hero-image path, .hero-image line')
  paths = Array.from(els).map((path: SVGPathElement) => {
  ...
  paths = sortBy(paths, (el) => el.length).reverse()
}
